...
Where $q(x_t \| x_{t-1})$ is the distribution that governs the target to noise transition. As we move from $0$ to $T$, the values of $\alpha_t$ are chosen so that their cumulative effect takes us from the target distribution at $0$ to a standard Gaussian at $T$ (the precise mechanics of this accumulation are covered in the DDPM section below). Note that the Gaussian Probability Path models the movement of the data between the initial and target distribution, but the Gaussian distribution at $0$ is not the target distribution itself. The entire transition from target to noise can be represented with the equation:
...
Training a model to learn $\theta$ is done using the Evidence Lower Bound (ELBO). This derivation is described in great detail in [^5], and I'll only repeat highlights here. The ELBO value we want to maximise is:
...
This derivation has been summarised from [^5] - further details, particularly around how the noise schedule terms $\alpha_t$ are accumulated can be found there. 

In DDPM [^2], they create a variant of this loss ($L_{\text{simple}}$) which is given by:
...
### Inference
To generate samples from $p^{\text{target}}_0$ we need to simulate the reverse process. DDPM and DDIM differ in their approaches here. In DDPM we iterate through $T..1$ and use the following equation:
...
Where $z \sim N(0, I)$ and $\sigma_t = \sqrt{1 - \alpha_t}$. This reverse update is a discretisation effectively; as $T \rightarrow \infty$ the forward noising process it reverses (i.e. $q(x_t \| x_{t-1})$ from earlier) converges to the following forward SDE [^4]:
...
DDPM's simulation process is based on the fact that the forwards and backwards equations are Markov Chains, that is, the value of $x_t$ depends only on $x_{t-1}$. DDIM extends DDPM to non-markovian equations. DDIM shares the training process with DDPM, however the reverse simulation equation is altered to:

$$

x_{t-1} = \sqrt{\frac{\bar{\alpha}_{t - 1}}{\bar{\alpha}_t}} (x_t - \sqrt{1 - \bar{\alpha}_t} \hat{\epsilon}_{\theta}(x_t, t)) + \sqrt{1 - \bar{\alpha}_{t-1} - \sigma_t^2} \hat{\epsilon}_{\theta}(x_t, t) + \sigma_t z

$$

In DDIM the parameter $\sigma\_t$ is actually a tunable parameter. When $\sigma\_t = 0$ the process becomes deterministic, while when $\sigma\_t = \sqrt{\frac{1 - \bar{\alpha}\_{t-1}}{1 - \bar{\alpha}\_t}} \sqrt{1 - \frac{\bar{\alpha}\_t}{\bar{\alpha}\_{t-1}}}$  we recover DDPM [^1]. 

The advantage of DDIM's approach is that the value of $T$ we use during simulation is arbitrary. In contrast, when sampling using DDPM's approach, we need to use the same $T$ as used during training. This means we can use DDIM's sampling process to generate samples from $p^{\text{target}}_0$ faster.

## Examples
So that's a whole bunch of math. Writing that out helps me get a grip on what is going on with Diffusion Models, but key to understanding something is implementing it. To that end, I've implemented some models on toy data using the approaches described above. 
...
I got Claude to find the best parameters for me for each shape, sweeping over the tunable $\sigma_t$ from the DDIM update above (which I'll refer to as the diffusion coefficient) and the number of steps $T$. It set up its own little temp script and ran some experiments, minimising the `chamfer distance` which I'd never heard of before, but it's measured like this:
...
| shape | diffusion_coef | steps | chamfer distance |
|---|---|---|---|
...
Different values of the diffusion coefficient don't appear to have a large impact on the final shape of the distribution, but you can see very different behaviour in the simulation process. With diffusion coefficient $= 0$ the process is deterministic and you can see the dots simply travel to their destination. The process for the circle is interesting as the dots travel to the centre then to their location on the circle. 

With diffusion coefficient $=0.5$ (second row) or using the DDPM setting (final row), we see that the simulation process is much more noisy right up to the final moment when the points congeal into the target distribution. Both processes appear similar.

Note that the plot below labels the diffusion coefficient as $\eta$.
...
### Value of $T$
Values of $T$ also don't really have a large impact on the final outcome. Lower values of $T$ seem to result in a less tight distribution. I think this makes sense as I'm not sure the DDIM reparameterisation made the simulation process exact for smaller $T$, just significantly more accurate.
...
## Conclusion
So that's a basic overview of diffusion models. They're kind of cool. It was fun to read into them and build them from scratch. Overall this was a pretty simple analysis using toy data. There are much more complex considerations when applying to more complex data such as images - for example, the choice of network architecture such as a U-net or transformer [^1]. Further, there are numerous papers on variations of sampling approaches and so on.
...
I found this approach to be quite good. Rather than delegating all my tasks to Claude, I used it as an information search tool. It was able to point out flaws and suggest alternative approaches / designs. This was valuable because it helped me learn how to do things better and kept me more engaged in the building process. For learning new things, I highly recommend using this approach.
...
## References
[^1]: Song, J., Meng, C., & Ermon, S. (2020). *Denoising Diffusion Implicit Models*. arXiv:2010.02502. https://arxiv.org/abs/2010.02502
[^2]: Ho, J., Jain, A., & Abbeel, P. (2020). *Denoising Diffusion Probabilistic Models*. arXiv:2006.11239. https://arxiv.org/abs/2006.11239
[^3]: Holderrieth, P., & Erives, E. (2025). *An Introduction to Flow Matching and Diffusion Models*. arXiv:2506.02070. https://arxiv.org/abs/2506.02070
[^4]: Song, Y., Sohl-Dickstein, J., Kingma, D. P., Kumar, A., Ermon, S., & Poole, B. (2021). *Score-Based Generative Modeling through Stochastic Differential Equations*. International Conference on Learning Representations (ICLR). https://openreview.net/forum?id=PxTIG12RRHS
[^5]: Luo, C. (2022). *Understanding Diffusion Models: A Unified Perspective*. arXiv:2208.11970. https://arxiv.org/abs/2208.11970