...
I find agents and sequential decision making to be a one of the most interesting areas of AI. This drew me to completing a masters thesis in reinforcement learning (RL) and spending much of my own time tinkering with these types of algorithms. RL, however, is sample inefficient and takes a long time to achieve an adequate level of performance. This is both in terms of time (especially without a GPU), and in terms of the number of observations (e.g. environment steps or episodes) required. 

As a result of my frustrations running slow agents on relatively simple environments, I was interested to see Yann LeCunn's oft quoted statement: <a href="https://drive.google.com/file/d/1BU5bV3X5w65DwSMapKcsr0ZvrMRU_Nbi/view"> "Abandon Reinforcement Learning in favour of Model Predictive Control"</a>. Model Predictive Control (MPC) uses a dynamics model and reward function to determine optimal actions. Using this approach, so long as you have a decent dynamics model you can achieve good performance on tasks really quickly. The key challenge and caveat being - you need a reasonale dynamics model. This may be available in domains such as physics where the world is well understood and predictable, but more challenging in general applications.

Even so, I started to explore MPC and how to implement it. A key challenge I found - and a motivation for writing this article - was that many resources on MPC were either too basic or too technical.  In the first case, only the general skeleton of the algorithm was described - which is easy enough to understand, but these left important gaps regarding how to actually _implement_ MPC. In the second case, exposition was very application specific, and often spent a great deal of effort describing dynamics models which made it difficult to understand how the MPC algorithm _actually_ worked. 

Eventually, after more digging, and (to be perfectly honest) asking ChatGPT, I was able to understand and implement some MPC algorithms which were application agnostic. The purpose of this article is to summarise some key MPC algorithms without going into detail about application specific dynamics. Below I'll summarise four key algorithms and then provide a brief demonstration of some implementations on a toy environment.

## MPC at a high-level
MPC is a relatively simple algorithm that involves taking an action and predicting the reward (or cost) of that action by simulating the outcome of the action using a model of your environment. The typical approach is to generate actions for k-steps into the future, run these actions through a dynamics model, and then evaluate the total reward of those actions. You run several of these rollouts and pick the first action from the trajectory that achieves the best reward or lowest cost. You repeat this everytime you need to make a decision, thus enabling you to choose actions which should maximise your reward over a forward window based on your knowledge of the environment.
...
3: \quad \text{Predict trajectory: } \hat{x}_{j+1} = f_\theta(\hat{x}_j, u_j) \text{ for } j = 0 \dots k-1 \\
4: \quad \text{Evaluate total reward: } J^{(i)} = \phi(\hat{x}_{k-1}) + \sum_{j=0}^{k-1} R(\hat{x}_j, u_j) \\
5: \textbf{end for} \\
...
3. How do you get this dynamics model $f$?
4. What is the reward model $R$?

MPC algorithms answer questions 1 and 2 above. They are designed to generate a set of actions, and then use the outputs of the dynamics and reward functions to optimise these actions. That is, they find the actions that maximise the reward (or minimise the cost) over the next k-steps in the future. Each algorithm discussed below has a different approach to doing this. I'll focus on these elements in my discussions.
...
where we apply a negative sign because we're talking about reward rather than cost, $x_{t}$, $u_{t}$ are the states and actions at time $t$, $P$ is a cost matrix for states (for example a penalty base on distance from a target state) and $Q$ is a cost matrix for actions (i.e. we might want to incentivise smaller actions). In any case, as the objective of my article is to avoid getting bogged down in the specifics of the environment, I'll assume these components are given.
...
5: \textbf{end for} \\
6: \text{Identify best: } i^* = \arg\max_i J^{(i)} \\
7: \textbf{return } u_t^* = \mathbf{u}^{(i^*)}_{0} \\
\hline
\end{array}
$$
...
CEM is a genetic algorithm or population-based method, which randomly samples actions and then refines those actions by aggregating the top-m or top-$\alpha$ percentile reward-achieving-trajectories[^{CEM}]. The algorithm works much like the random-shooting algorithm with the addition of the refinement process. 
...
and $Z$ is a normalising constant and $\lambda$ is a temperature parameter. Larger values of $\lambda$ result in a closer to uniform weighting. This weighting is derived from the information theoretic objectives of the MPPI algorithm [^MPPI].
...
\hline
1: \text{Sample } N \text{ noise sequences: } \epsilon^{(n)}_{0:k-1} \sim \mathcal{N}(0, \Sigma) \\
2: \textbf{for } \text{each sample } n = 1 \dots N \textbf{ do} \\
3: \quad \text{Perturb: } v^{(n)}_j = u_j + \epsilon^{(n)}_j \\
4: \quad \text{Roll out: } \hat{x}_{j+1} = f_\theta(\hat{x}_j, v^{(n)}_j) \\
5: \quad \text{Total Reward: } J^{(n)} = \phi(\hat{x}_{k-1}) + \sum_{j=0}^{k-1} R(\hat{x}_j, v^{(n)}_j) \\
6: \textbf{end for} \\
7: \text{Compute weights: } w^{(n)} = \frac{1}{Z} \exp \left( \frac{1}{\lambda} (J^{(n)} - \max(J)) \right) \\
8: \text{Update mean: } u_j \leftarrow u_j + \sum_{n=1}^{N} w^{(n)} \epsilon^{(n)}_j \text{ for } j = 0 \dots k-1 \\
...
$$
J_{t} = -(\phi(x_k) + \sum^{k-1}_{i = t+1}{\hat{x}_{i}R\hat{x}_{i}^{T} + u_{i}Qu_{i}^{T}})
$$ 

We can see that $J_{t}$ depends on the estimates of the state $\hat{x_{t}}$ which is given by:
...
3: \quad \text{Compute objective: } J = \phi(\hat{x}_{k-1}) + \sum_{j=0}^{k-1} R(\hat{x}_j, u_j) \\
...
Which adds the square of the final velocity ($v_{T}$) to each trajectory. This is important as it can stablise some algorithms (particularly GB-MPC) as they optimise the reward for the trajectory, but don't consider any future rewards. My approach is a bit hacky as technically the terminal reward should represent the reward over an infinite trajectory (i.e. basically what the value function does in RL), but to do this correctly, you need to factor in the environment dynamics to figure out a steady state. I didnt do this because firstly, I wasn't quite sure how to do this and, secondly, I wanted to treat the environment as a black-box as much as possible.

### The results
I ran each of the algorithms five times (to account for randomness) on a single landscape. Each algorithm has access to the exact dynamics of the environment - so they will know exactly what happens for any given action. This enables us to focus on the impact of the algorithms themselves.

I've used the below landscape. I like this landscape because the target location is on a slope, requiring the algorithm to maintain position while working against gravity.
...
## Reflections
The MPC algorithms above are able to solve the above environments in a single episode and within a few timesteps. Of course, this example was pretty simple and there are a number of elements worth exploring further.
